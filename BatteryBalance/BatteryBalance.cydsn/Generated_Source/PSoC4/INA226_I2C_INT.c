...
/*******************************************************************************
* Function Name: INA226_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(INA226_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef INA226_I2C_ISR_ENTRY_CALLBACK
    INA226_I2C_ISR_EntryCallback();
#endif /* INA226_I2C_ISR_ENTRY_CALLBACK */

#if (INA226_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = INA226_I2C_ACK_ADDR;
#endif /* (INA226_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */

    endTransfer = 0u; /* Continue active transfer */

    /* Calls customer routine if registered */
    if(NULL != INA226_customIntrHandler)
    {
        INA226_customIntrHandler();
    }

    if(INA226_CHECK_INTR_I2C_EC_MASKED(INA226_INTR_I2C_EC_WAKE_UP))
    {
        /* Mask-off after wakeup */
        INA226_SetI2CExtClkInterruptMode(INA226_NO_INTR_SOURCES);
    }
    ...
    */
    if(INA226_CHECK_I2C_FSM_MASTER)
    {
        #if(INA226_I2C_MASTER)
        {
            ...
            */
            if(INA226_CHECK_INTR_MASTER_MASKED(INA226_INTR_MASTER_I2C_BUS_ERROR))
            {
                INA226_mstrStatus |= (uint16) (INA226_I2C_MSTAT_ERR_XFER |
                                                         INA226_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = INA226_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(INA226_CHECK_INTR_MASTER_MASKED(INA226_INTR_MASTER_I2C_ARB_LOST))
            {
                INA226_mstrStatus |= (uint16) (INA226_I2C_MSTAT_ERR_XFER |
                                                         INA226_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = INA226_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(INA226_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(INA226_CHECK_I2C_MASTER_CMD(INA226_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    INA226_mstrStatus |= (uint16) (INA226_I2C_MSTAT_ERR_XFER |
                                                             INA226_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                INA226_mstrStatus |= (uint16) INA226_GET_I2C_MSTAT_CMPLT;

                #if(INA226_I2C_MULTI_MASTER_SLAVE)
                {
                    if(INA226_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(INA226_CHECK_I2C_MASTER_CMD(INA226_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            INA226_I2C_MASTER_CLEAR_START;

                            endTransfer = INA226_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!INA226_CHECK_INTR_SLAVE_MASKED(INA226_INTR_SLAVE_I2C_BUS_ERROR))
                               && INA226_CHECK_INTR_MASTER_MASKED(INA226_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = INA226_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            INA226_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            INA226_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */

                            /* Clean-up master interrupt sources */
                            INA226_ClearMasterInterruptSource(INA226_INTR_MASTER_ALL);

                            /* Disable data processing interrupts: they have to be cleared before */
                            INA226_SetRxInterruptMode(INA226_NO_INTR_SOURCES);
                            INA226_SetTxInterruptMode(INA226_NO_INTR_SOURCES);

                            INA226_state = INA226_I2C_FSM_IDLE;
                        }
                        else
                        {
                            /* Set I2C_FSM_EXIT_IDLE for BUS_ERR and ARB_LOST (that is really bus error) */
                            INA226_state = INA226_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (INA226_CHECK_I2C_FSM_SLAVE) */
    {
        #if(INA226_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or INA226_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(INA226_CHECK_INTR_SLAVE_MASKED(INA226_INTR_SLAVE_I2C_BUS_ERROR |
                                                        INA226_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(INA226_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    INA226_slStatus &= (uint8) ~INA226_I2C_SSTAT_RD_BUSY;
                    INA226_slStatus |= (uint8) (INA226_I2C_SSTAT_RD_ERR |
                                                          INA226_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    INA226_slStatus &= (uint8) ~INA226_I2C_SSTAT_WR_BUSY;
                    INA226_slStatus |= (uint8) (INA226_I2C_SSTAT_WR_ERR |
                                                          INA226_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(INA226_CHECK_I2C_FSM_MASTER)
    {
        #if(INA226_I2C_MASTER)
        {
            ...
            */
            if(INA226_CHECK_INTR_MASTER_MASKED(INA226_INTR_MASTER_I2C_STOP))
            {
                INA226_ClearMasterInterruptSource(INA226_INTR_MASTER_I2C_STOP);
            ...
            {
                if(INA226_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(INA226_CHECK_INTR_MASTER_MASKED(INA226_INTR_MASTER_I2C_NACK))
                    {
                        INA226_ClearMasterInterruptSource(INA226_INTR_MASTER_I2C_NACK);

                        INA226_mstrStatus |= (uint16) (INA226_I2C_MSTAT_ERR_XFER |
                                                                 INA226_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(INA226_CHECK_I2C_FSM_RD) /* Reading */
                        {
                            INA226_state = INA226_I2C_FSM_MSTR_RD_DATA;
                        }
                        else /* Writing */
                        {
                            INA226_state = INA226_I2C_FSM_MSTR_WR_DATA;
                            if(0u != INA226_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                INA226_SetTxInterruptMode(INA226_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(INA226_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(INA226_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(INA226_CHECK_INTR_RX_MASKED(INA226_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  INA226_mstrRdBufSize -
                                        (INA226_mstrRdBufIndex + INA226_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > INA226_I2C_FIFO_SIZE)
                            {
                                diffCount = INA226_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    INA226_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                INA226_mstrRdBufPtr[INA226_mstrRdBufIndex] = (uint8)
                                                                                        INA226_RX_FIFO_RD_REG;
                                INA226_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(INA226_CHECK_INTR_RX_MASKED(INA226_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            INA226_mstrRdBufPtr[INA226_mstrRdBufIndex] = (uint8) INA226_RX_FIFO_RD_REG;
                            INA226_mstrRdBufIndex++;

                            if(INA226_mstrRdBufIndex < INA226_mstrRdBufSize)
                            {
                                INA226_I2C_MASTER_GENERATE_ACK;
                            }
                            else
                            {
                               endTransfer = INA226_I2C_CMPLT_ANY_TRANSFER;
                        ...
                        INA226_ClearRxInterruptSource(INA226_INTR_RX_ALL);
                    }
                    else /* Writing */
                    {
                        /* INTR_MASTER_I2C_NACK :
                        ...
                        */
                        if(INA226_CHECK_INTR_MASTER_MASKED(INA226_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            INA226_mstrWrBufIndexTmp -= (INA226_GET_TX_FIFO_ENTRIES +
                                                                   INA226_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            INA226_mstrWrBufIndex = INA226_mstrWrBufIndexTmp;

                            INA226_mstrStatus |= (uint16) (INA226_I2C_MSTAT_ERR_XFER |
                                                                     INA226_I2C_MSTAT_ERR_SHORT_XFER);

                            INA226_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(INA226_CHECK_INTR_TX_MASKED(INA226_INTR_TX_EMPTY))
                        {
                            while(INA226_I2C_FIFO_SIZE != INA226_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(INA226_mstrWrBufIndexTmp < INA226_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(INA226_mstrWrBufIndexTmp == (INA226_mstrWrBufSize - 1u))
                                    {
                                        INA226_ClearTxInterruptSource(INA226_INTR_TX_UNDERFLOW);
                                        INA226_SetTxInterruptMode(INA226_INTR_TX_UNDERFLOW);
                                    }
                                 #endif /* (!INA226_CY_SCBIP_V0) */

                                    /* Put data into TX FIFO */
                                    INA226_TX_FIFO_WR_REG = (uint32) INA226_mstrWrBufPtr[INA226_mstrWrBufIndexTmp];
                                    INA226_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                            ...
                            INA226_ClearTxInterruptSource(INA226_INTR_TX_ALL);
                        #else
                            INA226_ClearTxInterruptSource(INA226_INTR_TX_EMPTY);
                        #endif /* (INA226_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(INA226_CHECK_INTR_TX_MASKED(INA226_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            INA226_mstrWrBufIndex = INA226_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(INA226_CHECK_I2C_MODE_NO_STOP(INA226_mstrControl))
                    {
                        /* On-going transaction is suspended: the ReStart is generated by the API request */
                        INA226_mstrStatus |= (uint16) (INA226_I2C_MSTAT_XFER_HALT |
                                                                 INA226_GET_I2C_MSTAT_CMPLT);

                        INA226_state = INA226_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
                        ...
                        */
                        INA226_I2C_MASTER_GENERATE_STOP;
                    }
                }
            }
    ...
    } /* (INA226_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(INA226_CHECK_I2C_FSM_SLAVE)
    {
        #if(INA226_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(INA226_CHECK_INTR_SLAVE_MASKED(INA226_INTR_SLAVE_I2C_NACK))
            {
                INA226_ClearSlaveInterruptSource(INA226_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (INA226_GET_TX_FIFO_ENTRIES + INA226_GET_TX_FIFO_SR_VALID);

                if(INA226_slOverFlowCount > diffCount) /* Overflow */
                {
                    INA226_slStatus |= (uint8) INA226_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    INA226_slRdBufIndexTmp -= (diffCount - INA226_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                INA226_slRdBufIndex = INA226_slRdBufIndexTmp;

                /* Clean-up TX FIFO */
                INA226_SetTxInterruptMode(INA226_NO_INTR_SOURCES);
                INA226_slOverFlowCount = 0u;
                INA226_CLEAR_TX_FIFO;

                /* Complete master reading */
                INA226_slStatus &= (uint8) ~INA226_I2C_SSTAT_RD_BUSY;
                INA226_slStatus |= (uint8)  INA226_I2C_SSTAT_RD_CMPLT;
                INA226_state     =  INA226_I2C_FSM_IDLE;
                
            #ifdef INA226_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                INA226_I2C_SlaveCompleteCallback();
            #endif /* INA226_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(INA226_CHECK_INTR_SLAVE_MASKED(INA226_INTR_SLAVE_I2C_WRITE_STOP))
            {
                INA226_ClearSlaveInterruptSource(INA226_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (INA226_I2C_CTRL_REG & INA226_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != INA226_GET_RX_FIFO_ENTRIES)
                    {
                        #if(INA226_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == INA226_GET_RX_FIFO_ENTRIES) &&
                               (INA226_CHECK_INTR_SLAVE_MASKED(INA226_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        INA226_slWrBufPtr[INA226_slWrBufIndex] = (uint8) INA226_RX_FIFO_RD_REG;
                        INA226_slWrBufIndex++;
                    }

                    INA226_DISABLE_SLAVE_AUTO_DATA;
                }

                if(INA226_CHECK_INTR_RX(INA226_INTR_RX_OVERFLOW))
                {
                    INA226_slStatus |= (uint8) INA226_I2C_SSTAT_WR_OVFL;
                }
                ...
                INA226_slStatus &= (uint8) ~INA226_I2C_SSTAT_WR_BUSY;
                INA226_slStatus |= (uint8)  INA226_I2C_SSTAT_WR_CMPLT;
                INA226_state     =  INA226_I2C_FSM_IDLE;
            ...
            */
        #if (INA226_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
            if (INA226_CHECK_INTR_SLAVE_MASKED(INA226_INTR_SLAVE_I2C_ADDR_MATCH |
        ...
        #endif /* (INA226_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                INA226_ClearI2CExtClkInterruptSource(INA226_INTR_I2C_EC_WAKE_UP);

                #if (INA226_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != INA226_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = INA226_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) INA226_RX_FIFO_RD_REG;
                        response = INA226_I2C_ACK_ADDR;
                    }
            ...
            #if (INA226_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == INA226_I2C_NAK_ADDR)
                {
                #if (!INA226_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    INA226_DISABLE_INTR_SLAVE(INA226_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!INA226_CY_SCBIP_V0) */

                    /* Clear address match and stop history */
                    INA226_ClearSlaveInterruptSource(INA226_INTR_SLAVE_ALL);

                    /* ACK the address byte */
                    INA226_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (INA226_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    if(INA226_CHECK_I2C_STATUS(INA226_I2C_STATUS_S_READ))
                    /* TX direction: master reads from slave */
                    {
                        INA226_SetTxInterruptMode(INA226_INTR_TX_EMPTY);

                        /* Set temporary index to address buffer clear from API */
                        INA226_slRdBufIndexTmp = INA226_slRdBufIndex;

                        /* Start master reading */
                        INA226_slStatus |= (uint8) INA226_I2C_SSTAT_RD_BUSY;
                        INA226_state     = INA226_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (INA226_slWrBufSize - INA226_slWrBufIndex);

                    #if (INA226_CY_SCBIP_V0)
                        if(diffCount < INA226_I2C_FIFO_SIZE)
                        /* Receive data: byte-by-byte */
                        {
                            INA226_SetRxInterruptMode(INA226_INTR_RX_NOT_EMPTY);
                        }
                        else
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == INA226_I2C_FIFO_SIZE)
                            {
                                ...
                                INA226_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                INA226_ENABLE_SLAVE_AUTO_DATA_ACK;
                                INA226_SetRxInterruptMode(INA226_INTR_RX_FULL);
                            }
                        }
                        ...
                        /* Start master reading */
                        INA226_slStatus |= (uint8) INA226_I2C_SSTAT_WR_BUSY;
                        INA226_state     = INA226_I2C_FSM_SL_WR;
                    }
                    ...
                    INA226_ENABLE_INTR_SLAVE(INA226_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!INA226_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    INA226_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(INA226_CHECK_INTR_RX_MASKED(INA226_INTR_RX_FULL))
            {
                ...
                diffCount =  INA226_slWrBufSize -
                            (INA226_slWrBufIndex + INA226_I2C_FIFO_SIZE);

                if(diffCount > INA226_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = INA226_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    INA226_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                INA226_ClearRxInterruptSource(INA226_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = INA226_RX_FIFO_RD_REG;

                if(INA226_slWrBufIndex < INA226_slWrBufSize)
                {
                    INA226_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    INA226_slWrBufPtr[INA226_slWrBufIndex] = (uint8) diffCount;
                    INA226_slWrBufIndex++;
                }
                ...
                INA226_ClearRxInterruptSource(INA226_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(INA226_CHECK_INTR_TX_MASKED(INA226_INTR_TX_EMPTY))
            {
                while(INA226_I2C_FIFO_SIZE != INA226_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(INA226_slRdBufIndexTmp < INA226_slRdBufSize)
                    /* Data from buffer */
                    {
                        INA226_TX_FIFO_WR_REG = (uint32) INA226_slRdBufPtr[INA226_slRdBufIndexTmp];
                        INA226_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        INA226_TX_FIFO_WR_REG = INA226_I2C_SLAVE_OVFL_RETURN;

                        if(INA226_slOverFlowCount <= INA226_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            INA226_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef INA226_I2C_ISR_EXIT_CALLBACK
    INA226_I2C_ISR_ExitCallback();
#endif /* INA226_I2C_ISR_EXIT_CALLBACK */
