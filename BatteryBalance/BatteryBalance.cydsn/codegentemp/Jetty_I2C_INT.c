...
/*******************************************************************************
* Function Name: Jetty_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(Jetty_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef Jetty_I2C_ISR_ENTRY_CALLBACK
    Jetty_I2C_ISR_EntryCallback();
#endif /* Jetty_I2C_ISR_ENTRY_CALLBACK */

#if (Jetty_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = Jetty_I2C_ACK_ADDR;
#endif /* (Jetty_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */

    endTransfer = 0u; /* Continue active transfer */

    /* Calls customer routine if registered */
    if(NULL != Jetty_customIntrHandler)
    {
        Jetty_customIntrHandler();
    }

    if(Jetty_CHECK_INTR_I2C_EC_MASKED(Jetty_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(Jetty_CHECK_I2C_FSM_MASTER)
    {
        #if(Jetty_I2C_MASTER)
        {
            ...
            */
            if(Jetty_CHECK_INTR_MASTER_MASKED(Jetty_INTR_MASTER_I2C_BUS_ERROR))
            {
                Jetty_mstrStatus |= (uint16) (Jetty_I2C_MSTAT_ERR_XFER |
                                                         Jetty_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = Jetty_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(Jetty_CHECK_INTR_MASTER_MASKED(Jetty_INTR_MASTER_I2C_ARB_LOST))
            {
                Jetty_mstrStatus |= (uint16) (Jetty_I2C_MSTAT_ERR_XFER |
                                                         Jetty_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = Jetty_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(Jetty_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(Jetty_CHECK_I2C_MASTER_CMD(Jetty_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    Jetty_mstrStatus |= (uint16) (Jetty_I2C_MSTAT_ERR_XFER |
                                                             Jetty_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                Jetty_mstrStatus |= (uint16) Jetty_GET_I2C_MSTAT_CMPLT;
                ...
                #if(Jetty_I2C_MULTI_MASTER_SLAVE)
                {
                    if(Jetty_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(Jetty_CHECK_I2C_MASTER_CMD(Jetty_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            Jetty_I2C_MASTER_CLEAR_START;

                            endTransfer = Jetty_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!Jetty_CHECK_INTR_SLAVE_MASKED(Jetty_INTR_SLAVE_I2C_BUS_ERROR))
                               && Jetty_CHECK_INTR_MASTER_MASKED(Jetty_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = Jetty_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            Jetty_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            Jetty_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            Jetty_state = Jetty_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (Jetty_CHECK_I2C_FSM_SLAVE) */
    {
        #if(Jetty_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or Jetty_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(Jetty_CHECK_INTR_SLAVE_MASKED(Jetty_INTR_SLAVE_I2C_BUS_ERROR |
                                                        Jetty_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(Jetty_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    Jetty_slStatus &= (uint8) ~Jetty_I2C_SSTAT_RD_BUSY;
                    Jetty_slStatus |= (uint8) (Jetty_I2C_SSTAT_RD_ERR |
                                                          Jetty_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    Jetty_slStatus &= (uint8) ~Jetty_I2C_SSTAT_WR_BUSY;
                    Jetty_slStatus |= (uint8) (Jetty_I2C_SSTAT_WR_ERR |
                                                          Jetty_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(Jetty_CHECK_I2C_FSM_MASTER)
    {
        #if(Jetty_I2C_MASTER)
        {
            ...
            */
            if(Jetty_CHECK_INTR_MASTER_MASKED(Jetty_INTR_MASTER_I2C_STOP))
            {
                Jetty_ClearMasterInterruptSource(Jetty_INTR_MASTER_I2C_STOP);
            ...
            {
                if(Jetty_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(Jetty_CHECK_INTR_MASTER_MASKED(Jetty_INTR_MASTER_I2C_NACK))
                    {
                        Jetty_ClearMasterInterruptSource(Jetty_INTR_MASTER_I2C_NACK);

                        Jetty_mstrStatus |= (uint16) (Jetty_I2C_MSTAT_ERR_XFER |
                                                                 Jetty_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(Jetty_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != Jetty_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                Jetty_SetTxInterruptMode(Jetty_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(Jetty_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(Jetty_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(Jetty_CHECK_INTR_RX_MASKED(Jetty_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  Jetty_mstrRdBufSize -
                                        (Jetty_mstrRdBufIndex + Jetty_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > Jetty_I2C_FIFO_SIZE)
                            {
                                diffCount = Jetty_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    Jetty_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                Jetty_mstrRdBufPtr[Jetty_mstrRdBufIndex] = (uint8)
                                                                                        Jetty_RX_FIFO_RD_REG;
                                Jetty_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(Jetty_CHECK_INTR_RX_MASKED(Jetty_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            Jetty_mstrRdBufPtr[Jetty_mstrRdBufIndex] = (uint8) Jetty_RX_FIFO_RD_REG;
                            Jetty_mstrRdBufIndex++;

                            if(Jetty_mstrRdBufIndex < Jetty_mstrRdBufSize)
                            {
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(Jetty_CHECK_INTR_MASTER_MASKED(Jetty_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            Jetty_mstrWrBufIndexTmp -= (Jetty_GET_TX_FIFO_ENTRIES +
                                                                   Jetty_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            Jetty_mstrWrBufIndex = Jetty_mstrWrBufIndexTmp;

                            Jetty_mstrStatus |= (uint16) (Jetty_I2C_MSTAT_ERR_XFER |
                                                                     Jetty_I2C_MSTAT_ERR_SHORT_XFER);
                        ...
                        */
                        else if(Jetty_CHECK_INTR_TX_MASKED(Jetty_INTR_TX_EMPTY))
                        {
                            while(Jetty_I2C_FIFO_SIZE != Jetty_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(Jetty_mstrWrBufIndexTmp < Jetty_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(Jetty_mstrWrBufIndexTmp == (Jetty_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    Jetty_TX_FIFO_WR_REG = (uint32) Jetty_mstrWrBufPtr[Jetty_mstrWrBufIndexTmp];
                                    Jetty_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            Jetty_ClearTxInterruptSource(Jetty_INTR_TX_EMPTY);
                        #endif /* (Jetty_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(Jetty_CHECK_INTR_TX_MASKED(Jetty_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            Jetty_mstrWrBufIndex = Jetty_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(Jetty_CHECK_I2C_MODE_NO_STOP(Jetty_mstrControl))
                    {
                        ...
                        Jetty_mstrStatus |= (uint16) (Jetty_I2C_MSTAT_XFER_HALT |
                                                                 Jetty_GET_I2C_MSTAT_CMPLT);

                        Jetty_state = Jetty_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
    ...
    } /* (Jetty_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(Jetty_CHECK_I2C_FSM_SLAVE)
    {
        #if(Jetty_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(Jetty_CHECK_INTR_SLAVE_MASKED(Jetty_INTR_SLAVE_I2C_NACK))
            {
                Jetty_ClearSlaveInterruptSource(Jetty_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (Jetty_GET_TX_FIFO_ENTRIES + Jetty_GET_TX_FIFO_SR_VALID);

                if(Jetty_slOverFlowCount > diffCount) /* Overflow */
                {
                    Jetty_slStatus |= (uint8) Jetty_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    Jetty_slRdBufIndexTmp -= (diffCount - Jetty_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                Jetty_slRdBufIndex = Jetty_slRdBufIndexTmp;
                ...
                Jetty_slStatus &= (uint8) ~Jetty_I2C_SSTAT_RD_BUSY;
                Jetty_slStatus |= (uint8)  Jetty_I2C_SSTAT_RD_CMPLT;
                Jetty_state     =  Jetty_I2C_FSM_IDLE;
                
            #ifdef Jetty_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                Jetty_I2C_SlaveCompleteCallback();
            #endif /* Jetty_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(Jetty_CHECK_INTR_SLAVE_MASKED(Jetty_INTR_SLAVE_I2C_WRITE_STOP))
            {
                Jetty_ClearSlaveInterruptSource(Jetty_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (Jetty_I2C_CTRL_REG & Jetty_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != Jetty_GET_RX_FIFO_ENTRIES)
                    {
                        #if(Jetty_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == Jetty_GET_RX_FIFO_ENTRIES) &&
                               (Jetty_CHECK_INTR_SLAVE_MASKED(Jetty_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        Jetty_slWrBufPtr[Jetty_slWrBufIndex] = (uint8) Jetty_RX_FIFO_RD_REG;
                        Jetty_slWrBufIndex++;
                    }

                    Jetty_DISABLE_SLAVE_AUTO_DATA;
                }

                if(Jetty_CHECK_INTR_RX(Jetty_INTR_RX_OVERFLOW))
                {
                    Jetty_slStatus |= (uint8) Jetty_I2C_SSTAT_WR_OVFL;
                }
                ...
                Jetty_slStatus &= (uint8) ~Jetty_I2C_SSTAT_WR_BUSY;
                Jetty_slStatus |= (uint8)  Jetty_I2C_SSTAT_WR_CMPLT;
                Jetty_state     =  Jetty_I2C_FSM_IDLE;
            ...
            */
        ...
        #endif /* (Jetty_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                Jetty_ClearI2CExtClkInterruptSource(Jetty_INTR_I2C_EC_WAKE_UP);

                #if (Jetty_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != Jetty_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = Jetty_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) Jetty_RX_FIFO_RD_REG;
                        response = Jetty_I2C_ACK_ADDR;
                    }
            ...
            #endif /* (Jetty_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        Jetty_slRdBufIndexTmp = Jetty_slRdBufIndex;

                        /* Start master reading */
                        Jetty_slStatus |= (uint8) Jetty_I2C_SSTAT_RD_BUSY;
                        Jetty_state     = Jetty_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (Jetty_slWrBufSize - Jetty_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == Jetty_I2C_FIFO_SIZE)
                            {
                                ...
                                Jetty_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                Jetty_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        Jetty_slStatus |= (uint8) Jetty_I2C_SSTAT_WR_BUSY;
                        Jetty_state     = Jetty_I2C_FSM_SL_WR;
                    }
                    ...
                    Jetty_ENABLE_INTR_SLAVE(Jetty_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!Jetty_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    Jetty_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(Jetty_CHECK_INTR_RX_MASKED(Jetty_INTR_RX_FULL))
            {
                ...
                diffCount =  Jetty_slWrBufSize -
                            (Jetty_slWrBufIndex + Jetty_I2C_FIFO_SIZE);

                if(diffCount > Jetty_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = Jetty_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    Jetty_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                Jetty_ClearRxInterruptSource(Jetty_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = Jetty_RX_FIFO_RD_REG;

                if(Jetty_slWrBufIndex < Jetty_slWrBufSize)
                {
                    Jetty_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    Jetty_slWrBufPtr[Jetty_slWrBufIndex] = (uint8) diffCount;
                    Jetty_slWrBufIndex++;
                }
                ...
                Jetty_ClearRxInterruptSource(Jetty_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(Jetty_CHECK_INTR_TX_MASKED(Jetty_INTR_TX_EMPTY))
            {
                while(Jetty_I2C_FIFO_SIZE != Jetty_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(Jetty_slRdBufIndexTmp < Jetty_slRdBufSize)
                    /* Data from buffer */
                    {
                        Jetty_TX_FIFO_WR_REG = (uint32) Jetty_slRdBufPtr[Jetty_slRdBufIndexTmp];
                        Jetty_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        Jetty_TX_FIFO_WR_REG = Jetty_I2C_SLAVE_OVFL_RETURN;

                        if(Jetty_slOverFlowCount <= Jetty_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            Jetty_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef Jetty_I2C_ISR_EXIT_CALLBACK
    Jetty_I2C_ISR_ExitCallback();
#endif /* Jetty_I2C_ISR_EXIT_CALLBACK */
